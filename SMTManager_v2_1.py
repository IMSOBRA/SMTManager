    ...
    def _validate(self):
        ...
        for item in processes:
            ...
            if name and name not in seen:
                ...
                cleaned.append(name)
        if not cleaned:
            cleaned = [normalize_process_name(x) for x in DEFAULT_CONFIG["game_processes"]]
...
class GameMonitor:
    ...
    def start(self):
        ...
        if HAS_WMI:
            ...
            logger.warning("WMI 모듈이 없어 폴링 모드로 시작합니다.")
    ...
    def _initial_scan(self):
        self._update_mask()
        names = [normalize_process_name(n) for n in self.config["game_processes"]]
        found = 0
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                ...
                if pname is None:
                    continue
                if normalize_process_name(pname) in names:
        ...
        try:
            ...
            while self.running:
                ...
                try:
                    ...
                    names = [normalize_process_name(n) for n in self.config["game_processes"]]
                    name_base = normalize_process_name(pname)
    ...
    def _poll_loop(self):
        ...
        while self.running:
            try:
                if self.config["enabled"]:
                    self._initial_scan()
                    stale = [pid for pid in self.processed_pids if not psutil.pid_exists(pid)]
                    for pid in stale:
                        del_name = self.processed_pids.pop(pid, None)
                        if del_name:
                            logger.info(f"🔄 종료 감지(폴링): {del_name} (PID: {pid})")
                    self._active_games = len(self.processed_pids)
                    if self._active_games > 0:
                        self._status = f"게임 {self._active_games}개 감지됨 (SMT OFF)"
                    else:
